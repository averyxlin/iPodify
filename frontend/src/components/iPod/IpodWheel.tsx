...
export function IpodWheel() {
  const { onMenu, onPlayPause, onUp, onDown, onCenter } = useIpodContext();
  return (
    <div className="w-56 h-56 bg-gradient-to-b from-gray-300 to-gray-500 rounded-full flex flex-col items-center justify-center relative shadow-inner">
      ...
      <button className="absolute bottom-6 left-1/2 -translate-x-1/2 text-gray-600" aria-label="Play/Pause">
        <FontAwesomeIcon icon={faPlay} size="sm" className="inline" />
        <FontAwesomeIcon icon={faPause} size="sm" className="inline ml-1" />
      </button>
